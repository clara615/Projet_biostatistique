...
Les variables sont les suivantes : - "statut_cas_temoin" : Précise le groupe de la personne Cas ou Témoin - "age", "sexe", "imc" - "region" : Donne la région parmis ("Centre-Val-de-Loire", "Hauts-de-France", "Ile-de-France").
...
$ 1.272 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | tabagisme=Fumeur actuel})}{odds(\text{statut_cas_temoin=1 | tabagisme=Ancien fumeur})}) \leq 2.130 $
...
$ -1.401 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | tabagisme=Jamais})}{odds(\text{statut_cas_temoin=1 | tabagisme=Ancien fumeur})}) \leq -0.283 $
...
$ -0.189 \leq ln(\frac{odds(\text{statut_cas_temoin = 1 | imc=x+1})}{odds(\text{statut_cas_temoin=1 | imc = x})}) \leq -0.085 $
...
$ 0.596 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | Exposition pro=faible})}{odds(\text{statut_cas_temoin=1 | exposition pro=aucune})}) \leq 1.450 $
...
$ 1.672 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | Exposition pro=importante})}{odds(\text{statut_cas_temoin=1 | exposition pro=aucune})}) \leq 2.637 $
...
$ 0.253 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | Exposition domestique=importante})}{odds(\text{statut_cas_temoin=1 | exposition domestique=aucune})}) \leq 0.988 $
...
#### Avec les varables quantitatives
...
Nous souhaitons trouvé avec le critère AIC le meilleur modèle. Nous réalisons donc un modèle complet avec toutes les variables. 
...
$ 0.883 \leq ln(\frac{odds(\text{statut_cas_temoin=1 | brochopneumo =oui})}{odds(\text{statut_cas_temoin=1 | bronchopneumo=non})}) \leq 1.942 $
...
Ainsi les personnes atteintes de cancer du poumon sont plus significativement plus exposées à la fumée dans le milieu professionnelle. Nous pouvons donc confirmer l'interpétation réalisée précédemment. 
